...
async function main() {
  ...
  const TOKEN = execSync(
    `curl -s -X POST https://fia.ct-cb1.workers.dev/api/auth/login -H "Content-Type: application/json" -d "{\\"loginId\\":\\"fia@admin.com\\",\\"password\\":\\"fia@123\\",\\"rememberMe\\":true}"`,
  ).toString()
  const token = JSON.parse(TOKEN).data.token
  execSync(
    `curl -s -X POST https://fia.ct-cb1.workers.dev/api/schools/upload -H "Authorization: Bearer ${token}" -F "file=@_tmp_live_test.xlsx"`,
  )
